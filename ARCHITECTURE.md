...
When the selected project is the configured self-repository, the agent also has a dedicated self-deployment tool. It runs tests, requests separate approval for commit, push, and deployment, then invokes the fixed read-only Compose helper to rebuild and recreate the bot service. The helper preserves the prior bot image under a rollback tag.
