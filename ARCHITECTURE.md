...
The session stores the optional legacy worker project, conversational computer context, compactable model input items, six short messages of router context, current-session usage totals, and at most one pending approval. Each direct OpenAI response also appends content-free usage metadata to a SQLite ledger in the host-persistent state directory. Ordinary messages can use constrained computer tools in the explicitly-mounted workspace. `/project` narrows that context to a directory below the workspace, `/new` starts over in the workspace root, `/usage` reports current-session, current UTC day, and lifetime recorded totals, and `/stop` forgets the session. Session changes never delete the durable ledger. The legacy worker remains available for the one-shot `/run` flow and future stronger isolation; its Codex CLI consumption is outside the ledger.
...
Main-model calls set explicit reasoning effort, response verbosity, and output limits by tier. Server-side compaction replaces older replay items after 32,000 rendered tokens by default. File listings, file reads, and command output are bounded, and exact-fragment edits avoid full-file generation when possible. Each router, main-model, and transcription response records input, cached input, cache writes, output, reasoning, and web-search use in memory and in the durable ledger. Ledger writes fail open so an accounting-storage fault does not discard a model reply; `/usage` identifies unavailable or incomplete durable accounting. A high-usage turn produces an operational warning but is not interrupted.
