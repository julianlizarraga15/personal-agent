...
Telegram long polling → authorized text input → ephemeral in-memory Telegram session → official Codex Python SDK thread → Bubblewrap workspace permissions + trusted API-Football and Git-publish MCP tools → optional exact-host HTTPS or exact-commit publication approval → debounced activity + rendered final answer + verified requested workspace images
