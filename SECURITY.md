...
- Public shell networking is blocked by default. Only an exact public HTTPS hostname on port 443 may produce an owner-only Telegram approval. Approval is destination-scoped, current-turn-only, expires after five minutes, and is cancelled by `/stop`, `/new`, or `/project`. Local/private hosts, IP literals, non-HTTPS traffic, filesystem escalation, and all other SDK approval methods fail closed. The sole pre-authorized local exception is `/run/api-football.sock`; the permission profile does not allow other Unix sockets.
- The optional API-Football credential exists only in the bot process. Its gateway hardcodes the official API-Sports TLS origin and authentication header, ignores proxy environment variables, allows only enumerated GET analytics endpoints and parameters, and rejects URLs, traversal, betting endpoints, predictions, malformed input, and oversized traffic before making an upstream request. Returned JSON is recursively secret-scrubbed, errors are stable and sanitized, and logs never include requests, responses, or the key.
