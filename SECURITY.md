...
- Never inspect, read, print, or expose the contents of `.env` under any circumstances. Use `.env.example` for configuration guidance. Enter `API_FOOTBALL_KEY` only in the private host `.env`, never in Telegram, source files, logs, or Codex configuration.
