...
- Operational logs are written to container stdout and intentionally exclude prompts, message text, file contents, command output, and credentials. Token counts, cache activity, web-search counts, and estimated cost are safe operational metadata. The durable usage database contains only timestamps, numeric Telegram user IDs, model/phase names, and those counts; it is created outside Git in the persistent state directory with owner-only file permissions when the filesystem permits. Restrict access to Docker logs and the state directory because paths, project names, and numeric Telegram user IDs may still be present.
