...
Configure only `git-publish.publish` with Codex per-tool `approval_mode="approve"`. This prevents Codex's generic MCP write-tool elicitation from declining the invocation before it reaches the application. The setting is not the publication approval: the MCP adapter remains keyless and the gateway below performs the sole authoritative, owner-visible decision.
...
The per-tool Codex override must remain scoped to `git-publish.publish`; applying it broadly would remove a useful independent review layer from unrelated MCP write tools. Publication approval delivery and outcome are logged without message bodies or credentials so an application rejection can be distinguished from a Codex-level tool rejection.
