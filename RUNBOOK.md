...
- Ask Codex to return two PNG originals “as files”; confirm both arrive as documents with unchanged names and bytes and never as photos. Also verify one PDF or binary file, mixed photo/file requests, and the shared four-attachment limit.
...
- Outbound file rejected: keep the regular file beneath the selected project and within `TELEGRAM_MAX_OUTPUT_DOCUMENT_BYTES`; protected `.env`, credential/private-key filenames, and private-key content cannot be sent. Logs deliberately omit the path and contents.
