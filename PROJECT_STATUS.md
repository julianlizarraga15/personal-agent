...
- Repository-scoped Git publication for `mental-models` is implemented and credential-boundary hardened in the local checkout. The full suite, bot image build, Compose validation, disposable local push, and built-container adversarial Git-filter/Bubblewrap test pass. Deploy-key provisioning, publication, deployment, and a live Telegram rejection/approval push remain.
...
- Publish/deploy these orchestration changes, then exercise rejection and approval of one non-sensitive `mental-models` commit through Telegram.
