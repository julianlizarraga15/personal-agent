...
The bot image includes `curl` for owner-requested public file downloads. Codex is instructed to pass a literal HTTPS URL to `curl`, allowing the sandbox to extract the exact destination for Telegram approval. Generic Python network clients and broad network escalation remain blocked because they do not provide the destination-scoped approval context.
...
If a turn needs to download a public file or declared dependencies, review the displayed hostname and approve only when it matches the task. The approval does not authorize a different host, later turn, non-HTTPS connection, shell escape, or broader filesystem access.
