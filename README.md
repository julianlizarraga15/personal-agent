...
You can send one Telegram photo or JPEG, PNG, WEBP, or non-animated GIF document in Codex mode. The optional caption tells Codex what to inspect; without one, the bot asks for a general description, visible text, errors, and actionable details. The bot reserves the user turn before downloading, verifies the declared and actual size, decodes the real format with decompression-bomb protection, and passes the validated bytes directly to the SDK as an in-memory data URL. The default limit is 10 MiB (`TELEGRAM_MAX_IMAGE_BYTES`). Albums, stickers, animation, video, and PDFs are not supported. Image context belongs to the ephemeral Codex conversation and disappears on `/new`, `/project`, `/stop`, or bot restart.
